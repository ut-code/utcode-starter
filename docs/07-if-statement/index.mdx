...
**if 文**を使うと、特定の条件下のみで実行される処理を書くことができます。

次のプログラムを実行すると、`子どもです。` と表示されますが、1 行目を `let age = 18;` に変更すると何も表示されなくなります。
...
次のプログラムを実行すると、`age` が `18` 未満の場合は `子どもです。` が表示されますが、`18` 以上の場合は `大人です。` が表示されます。
...
変数 `weather` が `晴れ` でなく `雨` の場合は、`今日は雨です。` と表示されます。
